...
Keyboard::Keyboard(): AWidget({0,600,540,360})
{
}
...
bool Keyboard::feed_event(const Event &event)
{
    ...
    for(auto ridx =0;ridx< key_rows.size();ridx++){
      ...
      for(auto idx= 0;idx<key_rows[ridx].size();idx++){
        ...
        if(touch->x < key_rect_info_.x) continue;
        if(touch->x > key_rect_info_.x+key_rect_info_.w) continue;
        if(touch->y < key_rect_info_.y) continue;
        if(touch->y > key_rect_info_.y+key_rect_info_.h) continue;
