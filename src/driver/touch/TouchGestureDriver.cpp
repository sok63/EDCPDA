...
void TouchGestureDriver::update(const sTouchInput& input) {
    ...
    else if (waitingForSecondTap_ && 
             (input.timestamp - lastTapTime_ > config_.doubleTapTimeMs)) {
        if (lastTap_.valid) {
            sTouchEvent event;
...
void TouchGestureDriver::handleTouchHold(const sTouchInput& input) {
    ...
    switch (state_) {
        case eTouchState::WAITING_FOR_LONG_PRESS:
            if (holdDuration >= config_.longPressTimeMs && 
                distance < config_.tapMaxDistance) {
                sTouchEvent event;
...
void TouchGestureDriver::handleTouchUp(const sTouchInput& input) {
    ...
    if (distance >= config_.swipeMinDistance && 
        touchDuration <= config_.swipeMaxTimeMs) {
    ...
    if (distance >= config_.dragMinDistance && 
        touchDuration > config_.swipeMaxTimeMs) {
        lastEventTime_ = input.timestamp;
        resetState();
        return;
    ...
    if (distance < config_.tapMaxDistance) {
        if (waitingForSecondTap_ && lastTap_.valid) {
            ...
            if (timeBetweenTaps < config_.doubleTapTimeMs && 
                distanceBetweenTaps < config_.doubleTapMaxDistance) {
                sTouchEvent event;
...
const char* TouchGestureDriver::getGestureName(eGestureType gesture) {
    switch (gesture) {
        case eGestureType::ONEF_TAP: return "Tap";
        case eGestureType::ONEF_DOUBLE_TAP: return "Double Tap";
        case eGestureType::ONEF_LONG_PRESS: return "Long Press";
        case eGestureType::ONEF_SWIPE_UP: return "Swipe Up";
        case eGestureType::ONEF_SWIPE_DOWN: return "Swipe Down";
        case eGestureType::ONEF_SWIPE_LEFT: return "Swipe Left";
        case eGestureType::ONEF_SWIPE_RIGHT: return "Swipe Right";
        case eGestureType::ONEF_DRAG_START: return "Drag Start";
        case eGestureType::ONEF_DRAG_MOVE: return "Drag Move";
        case eGestureType::ONEF_DRAG_END: return "Drag End";
        default: return "None";
    }
}
